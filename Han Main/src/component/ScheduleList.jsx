...
const ScheduleList = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      axios.get('http://localhost:8080/user/dashboard', { withCredentials: true })
        .then(res => {
          const myBookings = res.data.bookedClasses;
          if (myBookings) {
          
            const ids = myBookings.map(booking => booking.classId);
            setBookedClassIds(ids);
          }
        })
        .catch(err => {
          console.log("Belum ada data booking atau gagal menarik riwayat:", err);
        });
    }
  }, [user]);
  ...
  const handleBooking = async (classId) => {
    ...
    try {
      ...
      const errorMsg = error.response?.data || 'Gagal membooking kelas.';
      alert(errorMsg);

      if (errorMsg === 'Kamu sudah membooking kelas ini!') {
        setBookedClassIds(prev => [...prev, classId]);
      }
    }
  };
        ...
        {schedules.map((item) => {
          ...
          return (
            <div key={item.id} className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-lg hover:border-red-600/50 transition duration-300">
              ...
              >
                {isBooked ? 'SUDAH DIBOOKING' : 'BOOKING KELAS'}
              </button>
            </div>
          );
        })}
