...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const resClasses = await axios.get(`${import.meta.env.VITE_API}/api/classes`, { withCredentials: true });
        setClasses(resClasses.data);

        const resBookings = await axios.get(`${import.meta.env.VITE_API}/api/admin/bookings`, { withCredentials: true });
        setBookings(resBookings.data);

        const resUsers = await axios.get(`${import.meta.env.VITE_API}/api/admin`, { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${import.meta.env.VITE_API}/api/admin/classes`, formData, { withCredentials: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`${import.meta.env.VITE_API}/api/admin/classes/${id}`, { withCredentials: true });
  ...
  const handleUpdateStatus = async (bookingId, status) => {
    try {
      await axios.put(`${import.meta.env.VITE_API}/api/admin/bookings/${bookingId}`, { status }, { withCredentials: true });
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-lg mb-8">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <input type="time" name="startTime" placeholder="Waktu Mulai" value={formData.startTime} onChange={handleInputChange} className="bg-black text-white border border-zinc-700 rounded-lg px-4 py-2 focus:outline-none focus:border-red-600" required />
